...
# ---------------------------------------------------------------------
# 1. 문제 데이터셋 정의 (함수로 분리하여 관리)
# ---------------------------------------------------------------------
def get_questions_ba():
    # 65문항 리스트 생성
    ba_list = []
    for i in range(1, 66):
        ba_list.append({
            "id": i, "chapter": "고급데이터세미나", 
            "question": f"데이터 분석 실전 문항 {i}입니다. (이곳에 실제 65문항을 입력하세요)",
            "options": ["A", "B", "C", "D"], "answer": "A", 
            "explanation": "해설 준비중", "concept": "핵심 개념"
        })
    return ba_list

def get_questions_tech():
    # 30문항 리스트 (0X 및 단답형)
    return [
...
# 문제 선택 내비게이터 (슬라이더)
selected_id = st.sidebar.slider("문제 번호 이동", 1, total_q, value=st.session_state.q_idx + 1)
